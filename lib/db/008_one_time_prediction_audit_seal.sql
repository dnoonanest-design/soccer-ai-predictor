-- Seal newly inserted audit rows only after PostgreSQL has normalised every
-- signed value. This avoids false integrity failures from type/timestamp
-- round-trips while preserving append-only prediction evidence.
...
BEGIN
  ...
  IF OLD.audit_signature IS NULL
     ...
     AND NEW.signature_version = 'hmac-sha256-v2'
