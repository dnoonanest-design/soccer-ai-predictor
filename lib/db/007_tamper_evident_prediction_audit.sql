-- Tamper-evident performance audit.
-- The signing key remains outside PostgreSQL. Database edits therefore fail
-- application-side signature verification even for privileged database users.
...
CREATE INDEX IF NOT EXISTS idx_prediction_audit_signature
  ON prediction_audit_records(audit_signature, settled_at, captured_at DESC);
...
DROP TRIGGER IF EXISTS trg_protect_prediction_audit_record ON prediction_audit_records;
CREATE TRIGGER trg_protect_prediction_audit_record
BEFORE UPDATE OR DELETE ON prediction_audit_records
FOR EACH ROW EXECUTE FUNCTION protect_prediction_audit_record();
