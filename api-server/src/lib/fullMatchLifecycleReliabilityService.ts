...
export async function getFullMatchLifecycleReliabilityReport(limit = 30) {
  ...
  const [summary, historicalSummary, fixtures, runs] = await Promise.all([
    pool.query(
  ...
  const historicalCounts = Object.fromEntries(historicalSummary.rows.map((row) => [String(row.verdict), Number(row.count)]));
  ...
  return {
    ...
    historicalCounts: {
      collecting: Number(historicalCounts.collecting ?? 0),
      pass: Number(historicalCounts.pass ?? 0),
      warning: Number(historicalCounts.warning ?? 0),
      failed: Number(historicalCounts.failed ?? 0),
    },
    currentTelemetry: {
