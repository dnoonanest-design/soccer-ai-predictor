...
const CIRCUMSTANCE_CACHE_MS = Math.max(
  60_000,
  Number(process.env.CIRCUMSTANCE_CACHE_MS ?? 15 * 60_000),
);
const MIN_FACTOR_SAMPLE = Math.max(
  20,
  Number(process.env.MIN_FACTOR_SAMPLE ?? 40),
);
  ...
  try {
    ...
    if (!res.ok) {
      logger.warn(
        ...
        "circumstance API-Football request failed",
      );
      return fallback;
    }
...
function avg(values: Array<number | null | undefined>): number | null {
  ...
  return (
    Math.round((usable.reduce((a, b) => a + b, 0) / usable.length) * 100) / 100
  );
}
function formScore(form?: string | null): number | null {
  ...
  const score = chars.reduce(
    (sum, c, idx) => sum + (c === "W" ? 3 : c === "D" ? 1 : 0) * (1 + idx / 10),
    0,
  );
...
type ApiLineup = {
  ...
  formation?: string;
  startXI?: Array<{
    player?: { id?: number; name?: string; pos?: string; grid?: string };
  }>;
  substitutes?: Array<{
...
function eventCounts(events: MatchEvent[], side: "home" | "away") {
  ...
  return {
    ...
    red: mine.filter(
      (e) => e.type === "red_card" || e.type === "yellow_red_card",
    ).length,
    injurySubs: mine.filter(
      (e) =>
        e.type === "substitution" &&
        `${e.detail} ${e.comments ?? ""}`.toLowerCase().includes("injur"),
    ).length,
    goalContributors: new Set(
      mine
        .filter((e) => e.type === "goal")
        .flatMap((e) => [e.player, e.assist].filter(Boolean) as string[]),
    ).size,
  };
}
...
function teamLineup(lineups: ApiLineup[], teamId: number, teamName: string) {
  return (
    lineups.find((l) => l.team?.id === teamId) ??
    lineups.find(
      (l) => (l.team?.name ?? "").toLowerCase() === teamName.toLowerCase(),
    )
  );
}
function teamInjuries(injuries: ApiInjury[], teamId: number) {
  return injuries.filter((i) => i.team?.id === teamId);
}
function teamPlayers(players: ApiPlayerStats[], teamId: number) {
  return players.find((p) => p.team?.id === teamId);
}

function playerRating(stat: any): number | null {
  return n(stat?.games?.rating);
}
...
async function savePlayerFactors(
  match: Match,
  lineups: ApiLineup[],
  injuries: ApiInjury[],
  ...
  for (const side of ["home", "away"] as const) {
    ...
    const injuryNames = new Set(
      teamInjuries(injuries, team.id)
        .map((i) => (i.player?.name ?? "").toLowerCase())
        .filter(Boolean),
    );
    const injuryEventNames = new Set(
      events
        .filter(
          (e) =>
            e.team_side === side &&
    ...
    for (const missing of injuryNames) {
      if (
        !rows.some(
          (r) =>
            r.fixtureId === match.id &&
            r.teamSide === side &&
            r.playerName.toLowerCase() === missing,
        )
      ) {
        rows.push({
          fixtureId: match.id,
  ...
  for (const row of rows.slice(0, 80)) {
    try {
      await db
        .insert(playerMatchFactors)
        .values(row)
        .onConflictDoUpdate({
          target: [
            playerMatchFactors.fixtureId,
            playerMatchFactors.teamSide,
            playerMatchFactors.playerName,
          ],
      ...
      logger.warn(
        ...
        "failed to store player factor",
      );
    }
  }
}

export async function collectMatchCircumstances(
  match: Match,
  homeForm?: string | null,
  awayForm?: string | null,
) {
  const lineups = await fetchFootball<ApiLineup[]>(
    `/fixtures/lineups?fixture=${match.id}`,
    [],
  );
  const injuries = await fetchFootball<ApiInjury[]>(
    `/injuries?fixture=${match.id}`,
    [],
  );
  const playerStats = await fetchFootball<ApiPlayerStats[]>(
    `/fixtures/players?fixture=${match.id}`,
    [],
  );
  const events = await getMatchEvents(match.id, match.home_team.id).catch(
    () => [] as MatchEvent[],
  );
  ...
  const homeRatings = homeStats.map((p) =>
    playerRating(p.statistics?.[0] ?? {}),
  );
  const awayRatings = awayStats.map((p) =>
    playerRating(p.statistics?.[0] ?? {}),
  );
  const homeInfluence = homeStats.map((p) =>
    influenceFromStat(p.statistics?.[0] ?? {}),
  );
  const awayInfluence = awayStats.map((p) =>
    influenceFromStat(p.statistics?.[0] ?? {}),
  );
  ...
  const homeScore =
    Math.round(
      ((avg(homeInfluence) ?? 0) +
        (hForm ?? 50) * 0.25 +
        homeEvents.goalContributors * 4 -
        homeInjuries.length * 2 -
        homeEvents.red * 12 -
        homeEvents.injurySubs * 5) *
        10,
    ) / 10;
  const awayScore =
    Math.round(
      ((avg(awayInfluence) ?? 0) +
        (aForm ?? 50) * 0.25 +
        awayEvents.goalContributors * 4 -
        awayInjuries.length * 2 -
        awayEvents.red * 12 -
        awayEvents.injurySubs * 5) *
        10,
    ) / 10;
  ...
  const values = {
    ...
    homeStartersOut: homeInjuries.filter(
      (i) => i.player?.type || i.player?.reason,
    ).length,
    awayStartersOut: awayInjuries.filter(
      (i) => i.player?.type || i.player?.reason,
    ).length,
    ...
    homeStarPlayerRating:
      homeRatings
        .filter((x): x is number => x != null)
        .sort((a, b) => b - a)[0] ?? null,
    awayStarPlayerRating:
      awayRatings
        .filter((x): x is number => x != null)
        .sort((a, b) => b - a)[0] ?? null,
    homeFormScore: hForm,
...
export async function applyCircumstanceCalibration(
  match: Match,
  ...
  try {
    const rows = await db
      .select()
      .from(matchCircumstances)
      .where(eq(matchCircumstances.fixtureId, match.id))
      .limit(1);
    const c = rows[0];
    if (!c) return { ...probs, adjustment: null };
    const learnedRows = await db
      .select()
      .from(factorLearningInsights)
      .where(
        and(
          eq(factorLearningInsights.active, true),
          sql`${factorLearningInsights.factorName} IN ('circumstance_score_delta','red_card_delta','injury_delta','star_rating_delta','form_score_delta')`,
        ),
      )
      .orderBy(desc(factorLearningInsights.createdAt))
      .limit(10);
    const weights = new Map(
      learnedRows.map((r) => [r.factorName, Number(r.learnedWeight ?? 0)]),
    );
    const scoreDelta =
      Number(c.circumstanceScoreHome ?? 0) -
      Number(c.circumstanceScoreAway ?? 0);
    const redDelta = Number(c.awayRedCards ?? 0) - Number(c.homeRedCards ?? 0);
    const injuryDelta =
      Number(c.awayMissingPlayers ?? 0) +
      Number(c.awayInMatchInjuries ?? 0) * 2 -
      (Number(c.homeMissingPlayers ?? 0) +
        Number(c.homeInMatchInjuries ?? 0) * 2);
    const starDelta =
      Number(c.homeStarPlayerRating ?? 0) - Number(c.awayStarPlayerRating ?? 0);
    const formDelta =
      Number(c.homeFormScore ?? 50) - Number(c.awayFormScore ?? 50);
    const homeBoost = Math.max(
      -8,
      Math.min(
        8,
    ...
    return {
      ...normalise(
        probs.home + homeBoost,
        probs.draw + drawShift,
        probs.away + awayBoost,
      ),
      adjustment: {
        homeBoost,
        factors: { scoreDelta, redDelta, injuryDelta, starDelta, formDelta },
      },
    };
  } catch (err) {
    logger.warn(
      { err, fixtureId: match.id },
      "circumstance calibration failed",
    );
...
export async function analyzeCircumstanceInfluence() {
  ...
  const resultRows = Array.isArray((rows as any).rows)
    ? (rows as any).rows
    : (rows as any);
  let stored = 0;
  await db
    .update(factorLearningInsights)
    .set({ active: false })
    .where(eq(factorLearningInsights.active, true));
  for (const r of resultRows) {
    ...
    const confidence = Math.min(
      ...
      Math.round(
        Math.sqrt(sample) *
          Math.min(1, Math.abs(corr) + Math.abs(impact) / 5) *
          20,
      ),
    );
    await db.insert(factorLearningInsights).values({
      factorName: String(r.factor_name),
      factorGroup: String(r.factor_group),
      leagueId: r.league_id ?? null,
      sampleSize: sample,
      winRateWhenPositive: n(r.win_rate_when_positive),
      winRateWhenNegative: n(r.win_rate_when_negative),
      avgGoalDiffImpact: n(r.avg_goal_diff_impact),
      correlation: n(r.correlation),
      learnedWeight,
      confidence,
      notes: `Auto-learned from ${sample} settled matches. Positive values favour home; negative values favour away.`,
      active: true,
    });
    stored++;
  }
...
export async function getCircumstanceLearningReport() {
  const [recentInsights, recentCircumstances] = await Promise.all([
    ...
    db
      .select()
      .from(matchCircumstances)
      .orderBy(desc(matchCircumstances.updatedAt))
      .limit(20),
  ]);
