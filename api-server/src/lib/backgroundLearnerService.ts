...
import {
  ...
  settleTrackedBet,
} from "./predictionPlatformService";
import { logger } from "./logger";
import {
  analyzeCircumstanceInfluence,
  applyCircumstanceCalibration,
  collectMatchCircumstances,
  getCircumstanceLearningReport,
} from "./circumstanceLearningService";
import {
  getAiAwarenessReport,
  runAiAwarenessCycle,
} from "./aiAwareLearningService";
import {
  generateBiweeklyAiUpdate,
  getAiMemoryUpdateReport,
} from "./aiMemoryUpdateService";
...
const LIVE_INTERVAL_MS = Math.max(
  60_000,
  Number(process.env.BACKGROUND_LIVE_STATS_MS ?? 60_000),
);
const SETTLE_INTERVAL_MS = Math.max(
  10 * 60_000,
  Number(process.env.BACKGROUND_SETTLE_MS ?? 10 * 60_000),
);
const TRAIN_INTERVAL_MS = Math.max(
  6 * 60 * 60_000,
  Number(process.env.BACKGROUND_TRAIN_MS ?? 6 * 60 * 60_000),
);
const BIWEEKLY_UPDATE_INTERVAL_MS = Math.max(
  14 * 24 * 60 * 60_000,
  Number(process.env.BACKGROUND_BIWEEKLY_UPDATE_MS ?? 14 * 24 * 60 * 60_000),
);
const MAX_LIVE_MATCHES = Math.max(
  1,
  Number(process.env.BACKGROUND_MAX_LIVE_MATCHES ?? 12),
);
const MIN_AUTO_CALIBRATION_SAMPLE = Math.max(
  25,
  Number(process.env.MIN_AUTO_CALIBRATION_SAMPLE ?? 60),
);
...
function normalizeThreeWayPercent(home: number, draw: number, away: number) {
  const safe = [home, draw, away].map((p) =>
...
function buildValueEdges(
  ...
  const edge = (modelPct: number, decimalOdds: number | null) => {
    if (!decimalOdds || !Number.isFinite(decimalOdds) || modelPct <= 0)
      return null;
    const fairOdds = Math.round((100 / modelPct) * 100) / 100;
    const edgePct =
      Math.round((decimalOdds * (modelPct / 100) - 1) * 10000) / 100;
    return {
      bookmaker_odds: decimalOdds,
      fair_odds: fairOdds,
      edge_pct: edgePct,
      is_value: edgePct >= 5,
    };
...
async function recordJob(
  ...
  try {
    ...
    logger.warn(
      { err, jobName },
      "background learner: failed to record job run",
    );
  }
}
...
async function computeAndStoreMatch(match: Match) {
  ...
  const normalized = normalizeThreeWayPercent(
    applyCalibration(raw.home_win, "home", factors),
    applyCalibration(raw.draw, "draw", factors),
    applyCalibration(raw.away_win, "away", factors),
  );
  ...
  const valueEdges = buildValueEdges(
    match,
    adjusted.home,
    adjusted.draw,
    adjusted.away,
    ...
    if (liveMomentum?.pressure_alert) {
      await saveLiveAlert({
        ...
        pressureScore: Math.max(
          liveMomentum.home_pressure ?? 0,
          liveMomentum.away_pressure ?? 0,
        ),
...
export async function runLiveDeepStatCollection() {
  ...
  if (liveStatus === "running")
    return { skipped: true, reason: "live job already running" };
  liveStatus = "running";
    ...
    for (const match of liveMatches) {
      ...
      try {
        ...
        logger.warn(
          ...
          "background learner: live match failed",
        );
      }
    }
...
export async function runFinishedSettlement() {
  ...
  if (settleStatus === "running")
    return { skipped: true, reason: "settlement job already running" };
  ...
  try {
    ...
    for (const match of matches) {
      ...
      try {
        ...
        logger.warn(
          { err, fixtureId: match.id },
          "player stats collection failed",
        );
      }
      ...
      const openBets = await db
        ...
        .from(betTracker)
        .where(
          sql`${betTracker.fixtureId} = ${match.id} AND ${betTracker.status} = 'open'`,
        );
...
export async function runAutomaticRecalibration() {
  ...
  if (trainStatus === "running")
    return { skipped: true, reason: "training job already running" };
  trainStatus = "running";
  try {
    ...
    if (sampleSize < MIN_AUTO_CALIBRATION_SAMPLE) {
      await recordJob(
        "auto_recalibration",
        "success",
        Number(training.trainingRows ?? 0),
        Number(influence.stored ?? 0),
      );
      return {
        skipped: true,
...
export async function runBiweeklyAiUpdate(force = false) {
  if (!ENABLED) return { disabled: true };
  if (biweeklyStatus === "running")
    return { skipped: true, reason: "biweekly AI update already running" };
...
export function startBackgroundLearner() {
  ...
  liveTimer = setInterval(
    () =>
      runLiveDeepStatCollection().catch((err) =>
        logger.warn({ err }, "live background learner failed"),
      ),
    LIVE_INTERVAL_MS,
  );
  settleTimer = setInterval(
    () =>
      runFinishedSettlement().catch((err) =>
        logger.warn({ err }, "settlement background learner failed"),
      ),
    SETTLE_INTERVAL_MS,
  );
  trainTimer = setInterval(
    () =>
      runAutomaticRecalibration().catch((err) =>
        logger.warn({ err }, "recalibration background learner failed"),
      ),
    TRAIN_INTERVAL_MS,
  );
  biweeklyTimer = setInterval(
    () =>
      runBiweeklyAiUpdate(false).catch((err) =>
        logger.warn({ err }, "biweekly AI update failed"),
      ),
    BIWEEKLY_UPDATE_INTERVAL_MS,
  );
...
export async function getBackgroundLearnerStatus() {
  const recentRuns = await db
    ...
    .limit(20);
  const circumstanceLearning = await getCircumstanceLearningReport().catch(
    () => ({ recentInsights: [], recentCircumstances: [] }),
  );
  const aiAwareness = await getAiAwarenessReport().catch(() => ({
    activeModel: null,
    recentAudits: [],
    openImprovements: [],
  }));
  const aiMemoryUpdates = await getAiMemoryUpdateReport().catch(() => ({
    recentBiweeklyUpdates: [],
