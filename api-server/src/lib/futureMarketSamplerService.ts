...
const EMPTY_SPORT_KEY_COOLDOWN_MS = 6 * 60 * 60_000;
...
let budgetInitialization: Promise<void> | null = null;
const emptySportKeyUntil = new Map<string, number>();
...
export async function runFutureMarketSampler(): Promise<SamplerResult | { skipped: true; reason: string }> {
  ...
  try {
    refreshDailyBudget();
    await ensureDailyBudgetInitialized();
    ...
    const groups = Array.from(bySportKey.entries())
      ...
      .filter((group) => (emptySportKeyUntil.get(group.sportKey) ?? 0) <= nowMs)
      .sort((a, b) => {
        const priority = (group: typeof a) => {
          const competition = getTrackedCompetition(group.fixtures[0].league.id);
          if (competition?.kind === "uefa" || competition?.tier === 1) return 0;
          if (competition?.tier === 2) return 1;
          return 2;
        };
    ...
    for (const group of groups.slice(0, callsAllowed)) {
      ...
      emptySportKeyUntil.delete(group.sportKey);
...
function refreshDailyBudget() {
  ...
  if (today !== budgetDate) {
    ...
    budgetInitialization = null;
  }
}

async function ensureDailyBudgetInitialized() {
  refreshDailyBudget();
  if (budgetInitialized) return;
  if (!budgetInitialization) {
    budgetInitialization = reconstructDailyBudgetFromDb()
      .then((dbTotal) => {
        oddsCallsToday = Math.max(oddsCallsToday, dbTotal);
        budgetInitialized = true;
        logger.info({ oddsCallsToday }, "Daily odds sampler budget initialized from database");
      })
      .finally(() => {
        budgetInitialization = null;
      });
  }
  await budgetInitialization;
}
