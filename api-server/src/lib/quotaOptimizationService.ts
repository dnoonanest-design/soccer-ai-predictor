...
async function quotaOptimizedFetch(input: FetchInput, init?: RequestInit): Promise<Response> {
  ...
  const currentScheduleDate = responseFromStoredScheduleDate(parsed);
...
// Date responses are cached for hours to avoid repeatedly downloading the
// weekly schedule. Active fixture bundles are refreshed much more frequently
// and update `schedule`; serve that merged state so FT/live changes cannot be
// hidden behind the original pre-match NS response.
function responseFromStoredScheduleDate(parsed: URL): Response | null {
  if (parsed.pathname !== "/fixtures" || schedule.size === 0) return null;
  const date = parsed.searchParams.get("date");
  if (!date || !/^\\d{4}-\\d{2}-\\d{2}$/.test(date)) return null;

  const fixtures = Array.from(schedule.values()).filter((fixture) =>
    fixture.fixture?.date?.slice(0, 10) === date,
  );
  return synthetic(parsed, fixtures);
}
