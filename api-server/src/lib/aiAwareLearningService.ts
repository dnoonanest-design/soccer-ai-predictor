...
import { assertCoreAiFeatureVector, getCoreAiDataPolicyReport } from "./aiDataProvenancePolicy";
...
function clusterFromFeatures(row: any, fv: FeatureVector): string {
  ...
  const favGap = Math.abs((fv.home_win_prob ?? 0.33) - (fv.away_win_prob ?? 0.33));
...
export async function buildSimilarMatchMemory(limit = 500) {
  ...
  for (const row of rows.rows ?? []) {
    ...
    await db.insert(similarMatchMemory).values({
      ...
      notes: "Generated by AI-aware similar-match memory builder from internal predictor data only",
    }).catch(() => undefined);
    changed += 1;
  }
...
export async function findSimilarMatchesForFeatures(features: FeatureVector, leagueId?: number | null, maxResults = 20) {
  ...
  const ranked = candidates
    ...
    .map((r: any) => {
      ...
      assertCoreAiFeatureVector(stored, `stored_similar_match:${r.id ?? r.fixtureId ?? "unknown"}`);
      return { ...r, similarityScore: 1 - Math.min(1, distance(features, stored)) };
    })
...
export async function recalibrateFromSimilarMatchMemory() {
  ...
  for (const r of rows as any[]) {
    assertCoreAiFeatureVector((r.featureVectorJson ?? {}) as Record<string, unknown>, `recalibration_memory:${r.id ?? r.fixtureId ?? "unknown"}`);
  ...
  await db.insert(aiModelRegistry).values({
    ...
    active: true,
    notes: "Guarded AI-aware calibration. Learns only from the app's stored match data and settled results.",
  });
...
export async function getAiAwarenessReport() {
  ...
  return {
    ...
    explanation: "The core AI learns from the app's own stored prediction snapshots, match circumstances and settled results. Market intelligence and third-party forecasts are isolated from this learning path.",
  };
}
