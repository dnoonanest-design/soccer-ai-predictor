...
async function apiFetch(path: string): Promise<unknown> {
  ...
  try {
    ...
    if (!res.ok) {
      logger.warn({ status: res.status, url }, "enhanced: api-football failed");
      return null;
    }
...
async function fetchSquadStats(
  teamId: number,
  leagueId: number,
): Promise<Map<number, SquadPlayerStats>> {
...
function lookupByName(
  name: string,
  lookup: Map<string, SquadPlayerStats>,
): SquadPlayerStats | undefined {
...
function buildSpotlights(
  ...
  const topFouler = players.reduce((a, b) =>
    b.fouls_committed > a.fouls_committed ? b : a,
  );
  return {
    top_scorer: {
      name: topScorer.name,
      total: topScorer.goals,
      per_game: round2(topScorer.goals_per_game),
      prob: round2(poissonAtLeastOne(topScorer.goals_per_game)),
    },
    top_assister: {
      name: topAssister.name,
      total: topAssister.assists,
      per_game: round2(topAssister.assists_per_game),
      prob: round2(poissonAtLeastOne(topAssister.assists_per_game)),
    },
    top_fouler: {
      name: topFouler.name,
      total: topFouler.fouls_committed,
      per_game: round2(topFouler.fouls_per_game),
      prob: round2(poissonAtLeastOne(topFouler.fouls_per_game)),
    },
  };
}
...
async function fetchMatchEvents(
  ...
  if (cached) return cached;
  const data = (await apiFetch(`/fixtures/events?fixture=${fixtureId}`)) as
    | ApiEvent[]
    | null;
...
function computeSubstitutionImpacts(
  ...
  for (const sub of subs) {
    ...
    const rateIn = statsIn
      ? statsIn.goals_per_game + 0.5 * statsIn.assists_per_game
      : 0;
    const rateOut = statsOut
      ? statsOut.goals_per_game + 0.5 * statsOut.assists_per_game
      : 0;
...
async function fetchLineup(
  ...
  const [data, homeSquad, awaySquad] = await Promise.all([
    apiFetch(`/fixtures/lineups?fixture=${fixtureId}`) as Promise<
      ApiLineupEntry[] | null
    >,
  ...
  const toPlayers = (
    ...
    entry.startXI.map(({ player: p }) => {
      ...
      return {
        ...
        number: p.number,
        position: p.pos,
        goals_per_game: s?.goals_per_game ?? 0,
        assists_per_game: s?.assists_per_game ?? 0,
      };
    });
  ...
  const info: LineupInfo = {
    home: toPlayers(homeEntry, homeSquad),
    away: toPlayers(awayEntry, awaySquad),
    confirmed: true,
  };
  setCache(key, info);
  return info;
...
async function fetchInjuries(
  ...
  if (cached) return cached;
  const data = (await apiFetch(`/injuries?fixture=${fixtureId}`)) as
    | ApiInjury[]
    | null;
  if (!Array.isArray(data)) {
    setCache(key, []);
  ...
  const result: AbsentPlayer[] = data
    .filter((i) => relevant.has(i.team.id))
    .map((i) => ({
      name: i.player.name,
      team_id: i.team.id,
      type: i.injury.type,
...
function injuryFactor(
  ...
  const squadPlayers = Array.from(squadMap.values()).filter(
    (p) => p.appearances >= 3,
  );
  const squadAvgContrib =
    squadPlayers.length > 0
      ? squadPlayers.reduce(
          (sum, p) => sum + p.goals_per_game + 0.5 * p.assists_per_game,
          0,
        ) / squadPlayers.length
      : teamGpg / 11;
  ...
  return Math.max(0.7, 1.0 - totalImpact);
}
...
function lineupQualityFactor(
  ...
  const squadAvg =
    squadPlayers.reduce(
      (sum, p) => sum + p.goals_per_game + 0.4 * p.assists_per_game,
      0,
    ) / squadPlayers.length;
  if (squadAvg <= 0) return 1.0;

  // Average contribution rate of starters who appear in the squad database
  const startersInSquad = starters
    .map((s) => squadMap.get(s.id))
    .filter(Boolean) as SquadPlayerStats[];
  if (startersInSquad.length < 3) return 1.0;
  const starterAvg =
    startersInSquad.reduce(
      (sum, p) => sum + p.goals_per_game + 0.4 * p.assists_per_game,
      0,
    ) / startersInSquad.length;

  const ratio = starterAvg / squadAvg;
  return Math.min(1.2, Math.max(0.82, ratio));
}
...
function extendedPoissonMarkets(
  homeXG: number,
  awayXG: number,
): {
  over15: number;
  ...
  over35: number;
  btts: number;
  correctScores: CorrectScoreProbability[];
} {
  let over15 = 0,
    over25 = 0,
    over35 = 0,
  ...
  for (let h = 0; h <= MAX_GOALS; h++) {
    ...
    for (let a = 0; a <= MAX_GOALS; a++) {
      ...
      if (h <= 5 && a <= 5)
        scores.push({ score: `${h}-${a}`, probability: joint * 100 });
    }
  }
  ...
  return {
    ...
    correctScores: scores
      .slice(0, 6)
      .map((s) => ({ score: s.score, probability: round2(s.probability) })),
  };
}
...
function confidenceFromModel(
  ...
  return {
    label: score >= 72 ? "High" : score >= 55 ? "Medium" : "Low",
    score: round2(score),
  };
}
...
function buildReasons(opts: {
  ...
  if (Math.abs(xgDiff) >= 0.25)
    reasons.push(
      `${xgDiff > 0 ? opts.homeName || "Home" : opts.awayName || "Away"} has the stronger expected-goals profile.`,
    );
  if (opts.homeFormFactor - opts.awayFormFactor >= 0.04)
    reasons.push(`${opts.homeName || "Home"} has better recent form.`);
  if (opts.awayFormFactor - opts.homeFormFactor >= 0.04)
    reasons.push(`${opts.awayName || "Away"} has better recent form.`);
  if (opts.homeInjuryFactor <= 0.94)
    reasons.push(`${opts.homeName || "Home"} is weakened by absences.`);
  if (opts.awayInjuryFactor <= 0.94)
    reasons.push(`${opts.awayName || "Away"} is weakened by absences.`);
  if (opts.homeLineupFactor >= 1.06)
    reasons.push(
      `${opts.homeName || "Home"} lineup rates above squad average.`,
    );
  if (opts.awayLineupFactor >= 1.06)
    reasons.push(
      `${opts.awayName || "Away"} lineup rates above squad average.`,
    );
  if (opts.h2h && opts.h2h.matches >= 5)
    reasons.push(
      `Head-to-head sample included from the last ${opts.h2h.matches} meetings.`,
    );
  if (reasons.length === 0)
    reasons.push(
      "No strong edge detected; probabilities are mainly season-strength based.",
    );
...
function attackingIndex(
  stats: LiveTeamStatsInput | undefined,
  fallbackXG: number,
): number {
...
function liveMomentumFromEvents(
  ...
  const recent = events.filter((e) => {
    ...
    return (
      Number.isFinite(elapsed) && elapsed >= windowStart && elapsed <= minute
    );
  });
  ...
  const dominant =
    Math.abs(diff) < 10 ? "balanced" : diff > 0 ? "home" : "away";
  ...
  const momentumLabel =
    Math.abs(homeMomentumPct - awayMomentumPct) < 8
      ? "Balanced"
      : homeMomentumPct > awayMomentumPct
        ? "Home on top"
        : "Away on top";
  const alert =
    maxPressure >= 72 && Math.abs(diff) >= 15
  ...
  return {
    ...
    home_danger_score: round2(
      (liveStats?.home?.expected_goals_live ?? homeXG) * 25 +
        (liveStats?.home?.shots_inside_box ?? 0) * 4 +
        (liveStats?.home?.corners ?? 0) * 3,
    ),
    away_danger_score: round2(
      (liveStats?.away?.expected_goals_live ?? awayXG) * 25 +
        (liveStats?.away?.shots_inside_box ?? 0) * 4 +
        (liveStats?.away?.corners ?? 0) * 3,
    ),
...
function blendH2H(
  ...
  // Require a meaningful sample before allowing H2H to reach its maximum
  // influence. Five meetings made this noisy feature dominate too quickly.
  const w = Math.min(0.3, (h2h.matches / 20) * 0.3);
...
export async function getEnhancedPrediction(
  ...
  const [h2h, injuries, lineup, events, homeSquad, awaySquad] =
  ...
  const allInjuries =
    injuries.status === "fulfilled" ? injuries.value : ([] as AbsentPlayer[]);
  const lineupResult = lineup.status === "fulfilled" ? lineup.value : null;
  const eventsList =
    events.status === "fulfilled" ? events.value : ([] as ApiEvent[]);
  const homeSquadMap =
    homeSquad.status === "fulfilled"
      ? homeSquad.value
      : new Map<number, SquadPlayerStats>();
  const awaySquadMap =
    awaySquad.status === "fulfilled"
      ? awaySquad.value
      : new Map<number, SquadPlayerStats>();
  ...
  const adjHomeXG =
    baseHomeXG * homeFormFactor * homeLineupFactor * homeInjuryFactor;
  const adjAwayXG =
    baseAwayXG * awayFormFactor * awayLineupFactor * awayInjuryFactor;
  ...
  const hasScore =
    liveScoreHome != null && liveScoreAway != null && matchMinute != null;
  if (isLive && hasScore) {
  ...
  if (eventsList.length > 0) {
    ...
    if (substitutionImpacts.length > 0) {
      homeSubXgDelta = round2(
        substitutionImpacts
          .filter((s) => s.team === "home")
          .reduce((sum, s) => sum + s.xg_delta, 0),
      );
      awaySubXgDelta = round2(
        substitutionImpacts
