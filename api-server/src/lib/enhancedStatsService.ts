...
// Keep events aligned with the live fixture-stat cache/UI refresh. A two-minute
...
export interface LiveMomentum {
  ...
  source?: "live_stats" | "live_events" | "live_stats_and_events";
}
...
function hasLiveTelemetry(stats: LiveTeamStatsInput | undefined): boolean {
  if (!stats) return false;
  return [
    stats.possession, stats.shots_total, stats.shots_on_target, stats.corners,
    stats.fouls, stats.yellow_cards, stats.red_cards, stats.blocked_shots,
    stats.shots_inside_box, stats.expected_goals_live, stats.dangerous_attacks,
  ].some((value) => value !== null && value !== undefined && value !== "");
}
function attackingIndex(stats: LiveTeamStatsInput | undefined): number {
  if (!stats || !hasLiveTelemetry(stats)) return 0;
  const possession = pctToNumber(stats.possession) ?? 50;
  const passAccuracy = pctToNumber(stats.pass_accuracy) ?? 75;
  const liveXg = stats.expected_goals_live ?? 0;
...
export function liveMomentumFromEvents(events: ApiEvent[], homeTeamId: number, awayTeamId: number, minute: number | null, _homeXG: number, _awayXG: number, liveStats?: LiveMatchStatsInput): LiveMomentum | undefined {
  if (minute == null) return undefined;
  const hasStats = hasLiveTelemetry(liveStats?.home) || hasLiveTelemetry(liveStats?.away);
  const recent = events.filter((e) => Number(e.time.elapsed) >= Math.max(0, minute - 15) && Number(e.time.elapsed) <= minute);
  if (!hasStats && recent.length === 0) return undefined;
  const homeIndex = attackingIndex(liveStats?.home);
  const awayIndex = attackingIndex(liveStats?.away);
  // A small neutral base prevents an event-only feed producing a misleading 100/0 split.
  let homePressure = hasStats ? Math.min(82, homeIndex) : 10;
  let awayPressure = hasStats ? Math.min(82, awayIndex) : 10;
  for (const e of recent) {
    const age = Math.max(0, minute - Number(e.time.elapsed));
    const decay = Math.max(0.25, 1 - age / 20);
    const type = String(e.type ?? "").toLowerCase();
    const detail = String(e.detail ?? "").toLowerCase();
    const isCard = type === "card";
    const isRed = isCard && detail.includes("red");
    const attackingWeight = (type === "goal" ? 12 : type === "var" ? 4 : type.includes("subst") ? 1 : 2) * decay;
    const cardPenalty = (isRed ? 18 : 4) * decay;
    if (e.team.id === homeTeamId) {
      if (isCard) { homePressure -= cardPenalty; awayPressure += cardPenalty * 0.65; }
      else homePressure += attackingWeight;
    }
    if (e.team.id === awayTeamId) {
      if (isCard) { awayPressure -= cardPenalty; homePressure += cardPenalty * 0.65; }
      else awayPressure += attackingWeight;
    }
  }
  ...
  return {
    ...
    home_danger_score: round2((liveStats?.home?.expected_goals_live ?? 0) * 25 + (liveStats?.home?.shots_inside_box ?? 0) * 4 + (liveStats?.home?.corners ?? 0) * 3),
    away_danger_score: round2((liveStats?.away?.expected_goals_live ?? 0) * 25 + (liveStats?.away?.shots_inside_box ?? 0) * 4 + (liveStats?.away?.corners ?? 0) * 3),
    ...
    data_quality: hasStats ? "enhanced" : "basic",
    source: hasStats && recent.length ? "live_stats_and_events" : hasStats ? "live_stats" : "live_events",
  };
}

/** Live telemetry path independent of the pre-match prediction baseline. */
export async function getLiveMomentumSnapshot(
  fixtureId: number,
  homeTeamId: number,
  awayTeamId: number,
  minute: number | null,
  liveStats?: LiveMatchStatsInput,
): Promise<LiveMomentum | undefined> {
...
export async function getEnhancedPrediction(
  fixtureId: number, matchStatus: string,
  ...
  if (matchStatus !== "upcoming" && matchStatus !== "live") {
    throw new Error(`Prediction generation blocked for non-forecast status: ${matchStatus}`);
  }
  ...
  if (!isLive && learnedWeights.sampleSize >= 60 && learnedPriors) {
    ...
    finalAway = (1 - w) * finalAway + w * learnedPriors.away * 100;
    const total = finalHome + finalDraw + finalAway;
    finalHome = (finalHome / total) * 100;
    finalDraw = (finalDraw / total) * 100;
    finalAway = (finalAway / total) * 100;
  }
