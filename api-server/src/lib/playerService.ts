...
const INTERNATIONAL_LEAGUE_IDS = new Set([
  4, 5, 6, 7, 8, 9, 10, 152, 153, 154, 155, 156, 157, 158, 159, 160,
]);
    ...
    for (const teamData of data) {
      ...
      for (const playerData of (teamData.players || []) as ApiPlayerStat[]) {
        ...
        await updatePlayerProfile(p.id, p.name, {
          ...
          teamGoalsConceded,
          substitutedOff:
            minutesPlayed > 0 && minutesPlayed < 90 && !s.games?.substitute
              ? minutesPlayed
              : null,
        });
      }
...
async function updatePlayerProfile(
  ...
  for (let i = 0; i < last5.length; i++) {
    ...
    if (last5[i].assists)
      momentumScore += (last5[i].assists ?? 0) * 0.3 * weight;
  }
  ...
  const goalsProbability = Math.min(
    0.95,
    Math.max(0.02, momentumScore * 0.4 + (last10Goals / 10) * 0.6),
  );
  ...
  const firstHalf =
    recentMatches
      ...
      .reduce((a, b) => a + b, 0) / 5;
  const secondHalf =
    last5.map((m) => m.rating ?? 6).reduce((a, b) => a + b, 0) / 5;
  ...
  const avgMins =
    recentMatches.reduce((s, m) => s + (m.minutesPlayed ?? 0), 0) /
  ...
  const starterMatches = await db
    .select()
    .from(playerMatchStats)
    .where(and(eq(playerMatchStats.playerId, playerId)))
    .limit(50);
  const starterWins = starterMatches.filter(
    (m) => m.teamResult === "win",
  ).length;
  const teamWinRate = starterMatches.length
    ? starterWins / starterMatches.length
    : null;
  ...
  const values = {
    ...
    totalSuccessfulTackles:
      (base.totalSuccessfulTackles ?? 0) + match.successfulTackles,
    ...
    avgKeyPassesPerMatch:
      ((base.totalKeyPasses ?? 0) + match.keyPasses) / newTotalMatches,
    avgTacklesPerMatch:
      ((base.totalSuccessfulTackles ?? 0) + match.successfulTackles) /
      newTotalMatches,
    ...
    confidenceScore,
    attitudeScore,
    substitutedEarlyCount:
      (base.substitutedEarlyCount ?? 0) +
      (match.substitutedOff && match.substitutedOff < 60 ? 1 : 0),
    substitutedLateCount:
      (base.substitutedLateCount ?? 0) +
      (match.substitutedOff && match.substitutedOff >= 60 ? 1 : 0),
    ...
    winsAsStarter:
      (base.winsAsStarter ?? 0) +
      (match.isStarter && match.teamResult === "win" ? 1 : 0),
    clubMatches: (base.clubMatches ?? 0) + (match.isInternational ? 0 : 1),
    clubGoals:
      (base.clubGoals ?? 0) + (match.isInternational ? 0 : match.goals),
    internationalMatches:
      (base.internationalMatches ?? 0) + (match.isInternational ? 1 : 0),
    internationalGoals:
      (base.internationalGoals ?? 0) +
      (match.isInternational ? match.goals : 0),
