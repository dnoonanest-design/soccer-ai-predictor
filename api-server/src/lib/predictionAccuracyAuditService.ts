...
const ENGINE_REVISION =
  ...
  "unknown";
const AUDIT_SIGNING_KEY =
  process.env.PREDICTION_AUDIT_SIGNING_KEY?.trim() || null;
const SIGNATURE_VERSION = AUDIT_SIGNING_KEY
  ? "hmac-sha256-v3"
  : "sha256-dev-v3";
...
// PostgreSQL REAL is IEEE-754 float32. Sign the exact representation that the
// database stores, otherwise harmless float64-to-float32 rounding looks like
// tampering when a row is read back.
function float4(value: number) {
  ...
  return JSON.stringify(
    Object.keys(value)
      ...
      .reduce<Record<string, unknown>>((out, key) => {
        ...
        out[key] = item instanceof Date ? item.toISOString() : (item ?? null);
        return out;
      }, {}),
  );
}
...
function signPayload(value: Record<string, unknown>) {
  ...
  return AUDIT_SIGNING_KEY
    ? createHmac("sha256", AUDIT_SIGNING_KEY).update(payload).digest("hex")
    : createHash("sha256").update(payload).digest("hex");
}

function safeSignatureMatch(expected: string, actual: unknown) {
  if (typeof actual !== "string" || expected.length !== actual.length)
    return false;
  return timingSafeEqual(Buffer.from(expected), Buffer.from(actual));
}

function predictionSignaturePayload(row: Record<string, any>) {
  return {
    fixtureId: Number(row.fixture_id),
    leagueId: row.league_id == null ? null : Number(row.league_id),
    homeTeam: row.home_team,
    awayTeam: row.away_team,
    kickoffAt: row.kickoff_at == null ? null : new Date(row.kickoff_at),
    phase: row.phase,
    ...
    over25Prob: numberOrNull(row.over25_prob),
    bttsProb: numberOrNull(row.btts_prob),
    homeXg: numberOrNull(row.home_xg),
    awayXg: numberOrNull(row.away_xg),
    confidence: numberOrNull(row.confidence),
    pickConfidence: Number(row.pick_confidence),
    confidenceBand: row.confidence_band,
    predictedOutcome: row.predicted_outcome,
    circumstanceScoreHome: numberOrNull(row.circumstance_score_home),
    circumstanceScoreAway: numberOrNull(row.circumstance_score_away),
    homeFormScore: numberOrNull(row.home_form_score),
    awayFormScore: numberOrNull(row.away_form_score),
    capturedAt: new Date(row.captured_at),
  };
}

function settlementSignaturePayload(row: Record<string, any>) {
  return {
    auditSignature: row.audit_signature,
    actualOutcome: row.actual_outcome,
    scoreHome: Number(row.score_home),
    scoreAway: Number(row.score_away),
    correct: Boolean(row.correct),
    brierScore: numberOrNull(row.brier_score),
    logLoss: numberOrNull(row.log_loss),
    over25Actual: row.over25_actual,
    bttsActual: row.btts_actual,
    over25Correct: row.over25_correct,
    bttsCorrect: row.btts_correct,
    outcomeRecordedAt: new Date(row.outcome_recorded_at),
  };
}
...
function toMatch(fixture: FutureFixture): Match | null {
  ...
  return {
    ...
    league_name:
      fixture.league?.name ??
      getTrackedCompetition(leagueId)?.name ??
      "Unknown",
    league_logo: fixture.league?.logo ?? null,
    country:
      fixture.league?.country ?? getTrackedCompetition(leagueId)?.country ?? "",
    home_team: {
      id: homeId,
...
async function getUpcomingAuditMatches(now: Date): Promise<Match[]> {
  ...
  for (const date of dateKeysBetween(now, end)) {
    try {
      ...
      const response = await fetchFootball(
...
async function computeAuditPrediction(
  ...
  // Audits capture forecasts before/during play and settle them separately.
...
async function existingCheckpointKeys(fixtureIds: number[]) {
  ...
  return new Set(
    result.rows.map(
      (row) => `${Number(row.fixture_id)}:${String(row.checkpoint)}`,
    ),
  );
}
...
async function insertAuditRecord(
  ...
  const stored = {
    home: float4(prediction.home),
    draw: float4(prediction.draw),
    away: float4(prediction.away),
    over25: float4OrNull(prediction.over25),
    btts: float4OrNull(prediction.btts),
    homeXg: float4OrNull(prediction.homeXg),
    awayXg: float4OrNull(prediction.awayXg),
    confidence: float4OrNull(prediction.confidence),
    pickConfidence: float4(pickConfidence),
    circumstanceScoreHome: float4OrNull(prediction.circumstanceScoreHome),
    circumstanceScoreAway: float4OrNull(prediction.circumstanceScoreAway),
    homeFormScore: float4OrNull(prediction.homeFormScore),
    awayFormScore: float4OrNull(prediction.awayFormScore),
  };
  ...
  try {
    ...
    const result = await client.query(
     ...
     RETURNING *`,
      [
        ...
        stored.awayXg,
        stored.confidence,
        stored.pickConfidence,
        confidenceBand,
        predictedOutcome,
        stored.circumstanceScoreHome,
        stored.circumstanceScoreAway,
        stored.homeFormScore,
    ...
    const seal = await client.query(
      `UPDATE prediction_audit_records
          SET audit_signature = $2, signature_version = $3, sealed_at = NOW()
        WHERE id = $1 AND audit_signature IS NULL`,
      [Number(inserted.id), auditSignature, SIGNATURE_VERSION],
    );
    if ((seal.rowCount ?? 0) !== 1)
      throw new Error("prediction audit record could not be sealed");
    await client.query("COMMIT");
    return true;
  } catch (err) {
...
async function capturePrematchCheckpoints(now: Date) {
  ...
  const existing = await existingCheckpointKeys(
    matches.map((match) => match.id),
  );
  const due = matches
    .map((match) => ({
      match,
      checkpoint: prematchCheckpoint(
  ...
  for (const { match, checkpoint } of due) {
    try {
      ...
      const prediction = await computeAuditPrediction(
        match,
        includeCircumstances,
      );
      if (
        prediction &&
      ...
      errors++;
      logger.warn(
        { err, fixtureId: match.id, checkpoint },
        "prediction audit prematch capture failed",
      );
    }
  }
...
async function captureLiveCheckpoints() {
  ...
  try {
    liveMatches = (await getAllMatches(null, "live")).filter((match) =>
      isTrackedLeague(match.league_id),
    );
  } catch (err) {
  ...
  for (const { match, checkpoint } of due) {
    try {
      ...
      logger.warn(
        { err, fixtureId: match.id, checkpoint },
        "prediction audit live capture failed",
      );
    }
  }
  return { candidates: due.length, captured, errors };
...
export async function settlePredictionAuditRecords() {
  const pending = await pool.query(
    `SELECT a.id, a.home_win_prob, a.draw_prob, a.away_win_prob,
            ...
            a.audit_signature, o.outcome, o.score_home, o.score_away,
  ...
  for (const row of pending.rows) {
    ...
    const over25Predicted =
      over25Prob == null ? null : toUnitProbability(over25Prob) >= 0.5;
    const bttsPredicted =
      bttsProb == null ? null : toUnitProbability(bttsProb) >= 0.5;
    const outcomeRecordedAt = new Date(row.outcome_recorded_at);
    const storedBrierScore = float4(scores.brierScore);
    const storedLogLoss = float4(scores.logLoss);
    // Persist first, then sign the row PostgreSQL actually stored. PostgreSQL
    // REAL/timestamp normalisation can change the pre-write JS representation
    // and caused valid v2 settlements to fail their later verification.
    const client = await pool.connect();
    try {
      await client.query("BEGIN");
      ...
      if ((storedResult.rowCount ?? 0) === 0) {
        await client.query("ROLLBACK");
        continue;
      }

      const storedRow = storedResult.rows[0];
      const settlementSignature = signPayload(
        settlementSignaturePayload(storedRow),
      );
      const sealResult = await client.query(
        `UPDATE prediction_audit_records
            SET settlement_signature = $2, settled_at = NOW()
          WHERE id = $1 AND settled_at IS NULL`,
        [Number(row.id), settlementSignature],
      );
      if ((sealResult.rowCount ?? 0) !== 1) {
        throw new Error("prediction audit settlement could not be sealed");
      }
      await client.query("COMMIT");
      settled++;
    } catch (err) {
...
export async function runPredictionAccuracyAudit(): Promise<
  AuditRunResult | { skipped: true; reason: string }
> {
  if (!ENABLED)
    return { skipped: true, reason: "prediction accuracy audit disabled" };
  if (running)
    return {
      skipped: true,
      reason: "prediction accuracy audit already running",
    };
  running = true;
  lastError = null;
...
export async function verifyPredictionAuditIntegrity() {
  const result = await pool.query(`
    SELECT a.*,
           ...
           o.score_away AS current_score_away
      FROM prediction_audit_records a
      LEFT JOIN match_outcomes o ON o.fixture_id = a.fixture_id
     ORDER BY a.id ASC
  `);
  ...
  for (const row of result.rows) {
    ...
    if (row.signature_version !== "hmac-sha256-v3") {
      legacySigned++;
      continue;
    }
    ...
    const settlementValid =
      ...
      (row.settlement_signature &&
        ...
        safeSignatureMatch(
          signPayload(settlementSignaturePayload(row)),
          row.settlement_signature,
        ));
    ...
    const probabilitiesValid =
      ...
      Math.abs(probabilityTotal - 100) <= 0.15;
    ...
    if (!probabilitiesValid) invalidProbability++;
    ...
    status: !AUDIT_SIGNING_KEY
      ...
      : invalid > 0
        ...
        : validIds.length === 0
          ? "collecting"
          : "verified",
    signing: AUDIT_SIGNING_KEY ? "HMAC-SHA256" : "unsigned-development-hash",
    checked,
    verified: validIds.length,
    unsigned,
    legacySigned,
    invalid,
    latePrematch,
...
export async function getPredictionAccuracyAuditReport() {
  ...
  const [
    ...
    pool.query(
      `
      SELECT phase, COUNT(*)::int AS samples,
        ...
        FROM prediction_audit_records
       WHERE settled_at IS NOT NULL AND ${certifiedFilter}
  ...
  const rawCertified = certified.rows[0] ?? {};
  const settled = Number(rawCertified.settled ?? 0);
  const dataMaturity =
  ...
  return {
    ...
    methodology: {
      ...
      scoreScale: {
        ...
        brierScore:
          "lower is better; 3-way random baseline is approximately 0.667",
    ...
    integrity: (({ validIds: _validIds, ...publicIntegrity }) =>
      publicIntegrity)(integrityResult),
    certified: {
      captured: Number(rawCertified.captured ?? 0),
      ...
      over25Accuracy: toMetricNumber(rawCertified.over25_accuracy),
      bttsAccuracy: toMetricNumber(rawCertified.btts_accuracy),
    },
    overall: {
    ...
    byPhase: mapMetricRows(byPhase.rows),
    byLeague: mapMetricRows(byLeague.rows).map((row) => ({
      ...
      league:
        row.league_id == null
          ? "Unknown"
          : (getTrackedCompetition(Number(row.league_id))?.name ??
            `League ${row.league_id}`),
      country:
        row.league_id == null
          ? null
          : (getTrackedCompetition(Number(row.league_id))?.country ?? null),
    })),
    ...
    recent: recent.rows.map((row) => {
      const {
        ...
        settlement_signature: _settlementSignature,
      ...
      return {
        ...
        integrity_status: certifiedIds.includes(Number(row.id))
          ? "verified"
          : auditSignature && signatureVersion === "hmac-sha256-v2"
            ? "invalid"
            : auditSignature
              ? "legacy-signed"
              : "legacy-unsigned",
