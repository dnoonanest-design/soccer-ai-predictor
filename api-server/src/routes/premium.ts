...
import { isTrackedLeague as isLeagueFocused } from "../lib/leagueConfig";
import { waitForRateLimit } from "../lib/rateLimiter";
...
router.get("/matches/:match_id/probability-history", async (req, res) => {
  ...
  try {
    ...
    const explanationRows = await db
      .select({
        summary: aiLearningMemory.summary,
        evidenceJson: aiLearningMemory.evidenceJson,
      })
    ...
    return res
      .status(500)
      .json({ error: "Failed to fetch probability history" });
  }
});
...
router.get("/teams/:team_id/profile", async (req, res) => {
  ...
  const leagueId = req.query.league_id
    ? safeInt(req.query.league_id, 1, 99_999)
    : null;
  if (teamId === null)
    return res
      .status(400)
      .json({ error: "Invalid team_id: must be a positive integer" });
  try {
    ...
    const teamName = refMatch
      ? refMatch.home_team.id === teamId
        ? refMatch.home_team.name
        : refMatch.away_team.name
      : `Team ${teamId}`;
...
router.get("/fixtures/upcoming", async (req, res) => {
  ...
  if (!API_KEY)
    return res.json({
      fixtures: [],
      note: "API key required for multi-day fixtures",
    });
    ...
    for (let i = 0; i < days; i++) {
      ...
      try {
        ...
        if ((fetchErr as any)?.name === "AbortError") {
          ...
          logger.warn(
            { err: fetchErr, dateStr },
            "upcoming fixtures: fetch error",
          );
        }
      } finally {
        clearTimeout(timer);
      ...
      let list = fixtures.filter((f: any) =>
        leagueId
          ...
          : isLeagueFocused(Number(f.league?.id ?? 0)),
      );
  ...
  try {
    ...
    async function scoreMatch(match: (typeof candidateMatches)[0]) {
      try {
        ...
        if (!result.home.matches_played || !result.away.matches_played)
          return null;
        ...
        const computeEdge = (modelPct: number, decOdds: number | null) => {
          ...
          const edgePct =
            Math.round(((decOdds * modelPct) / 100 - 1) * 10000) / 100;
          return {
            model_prob: Math.round(modelPct * 10) / 10,
            fair_odds: Math.round(fairOdds * 100) / 100,
            bookmaker_odds: decOdds,
            edge_pct: edgePct,
            is_value: edgePct >= minEdge,
          };
        };
    ...
    const valueMatches = results
      ...
      .sort(
        (a: any, b: any) =>
          (b.best_edge?.edge_pct ?? 0) - (a.best_edge?.edge_pct ?? 0),
      );
...
router.get("/watchlist", async (_req, res) => {
  try {
    const rows = (await db.execute(
      sql`SELECT id, fixture_id, alert_rules, created_at FROM user_watchlist ORDER BY created_at DESC LIMIT 100`,
    )) as any;
...
router.delete("/watchlist/:fixture_id", async (req, res) => {
  ...
  try {
    await db.execute(
      sql`DELETE FROM user_watchlist WHERE fixture_id = ${fixtureId}`,
    );
  ...
  try {
    const rows = (await db.execute(sql`
      ...
      WHERE mp.is_live = false
        ...
        AND mp.created_at < mp.kickoff_at
        AND mp.updated_at < mp.kickoff_at
    `)) as any;
    ...
    const improvement =
      accuracy != null
        ? Math.round((accuracy - randomBaseline) * 10) / 10
        : null;
    ...
    return res.json({
      ...
      confidence_description:
        accuracy == null
          ? "Not enough data yet"
          : accuracy >= 55
            ? "Strong performance — meaningfully above the 33% random baseline"
            : accuracy >= 48
              ? "Good performance — above random baseline with room to improve"
              : accuracy >= 40
                ? "Developing — accumulating data to improve calibration"
