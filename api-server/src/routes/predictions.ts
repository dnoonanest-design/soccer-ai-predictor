...
import { canonicalPrematchAuditCte } from "../lib/canonicalPrematchAudit";
import { verifyPredictionAuditIntegrity } from "../lib/predictionAccuracyAuditService";
...
async function readPredictions(start: Date, end: Date) {
  ...
  const canonicalCte = canonicalPrematchAuditCte("$3");
  const result = await pool.query(
    `WITH ${canonicalCte}, latest_audit AS (
       SELECT
     ...
     SELECT
       ...
       a.engine_revision,
       CASE WHEN a.fixture_id IS NOT NULL THEN 'verified_audit_checkpoint'
            ELSE 'legacy_prediction_fallback' END AS source,
