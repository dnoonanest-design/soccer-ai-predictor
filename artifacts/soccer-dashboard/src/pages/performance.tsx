...
export default function Performance() {
  ...
  return <div className="space-y-5 max-w-[1500px] mx-auto">
    ...
    {intel && <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3"><Metric label="Latest day" value={latestTrend ? pct(latestTrend.accuracy) : "—"} note={latestTrend ? `${latestTrend.samples} certified checkpoints` : "Collecting"} /><Metric label="Manchester Rule" value={manchesterTagged ? pct(manchesterTagged.accuracy) : "—"} note={manchesterTagged ? `${manchesterTagged.samples} tagged samples` : "Awaiting tagged settlements"} /><Metric label="Market sample" value={intel.marketBenchmark.samples} note="External benchmark only" /><Metric label="Market movement" value={intel.marketBenchmark.averageMarketMovementPctPoints == null ? "—" : `${intel.marketBenchmark.averageMarketMovementPctPoints.toFixed(2)} pts`} note="Average price movement" /><Metric label="Model-market gap" value={intel.marketBenchmark.averageModelMarketGapPctPoints == null ? "—" : `${intel.marketBenchmark.averageModelMarketGapPctPoints.toFixed(2)} pts`} note="Never fed into core model" /></div>
      ...
      <div className="grid lg:grid-cols-2 gap-4"><BreakdownTable title="Lineup Confirmation Impact" rows={intel.byLineupState} label={(row) => (row.group_name ?? "unknown").replaceAll("-", " ")} /><BreakdownTable title="Manchester Rule Contribution" rows={intel.manchesterRule} label={(row) => (row.group_name ?? "unknown").replaceAll("-", " ")} /></div>
